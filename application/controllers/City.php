    ...
    */
    ...
    function index()
    {
        ...
        {        
            ...
            $count = $this->city_model->userListingCount($searchText);

			$returns = $this->paginationCompress ( "userListing/", $count, 10 );
            
            //$data['userRecords'] = $this->city_model->userListing($searchText, $returns["page"], $returns["segment"]);
            $data['userRecords'] = $this->city_model->userListing($searchText, $returns["page"], $returns["segment"]);
            $data['states'] = $this->city_model->showStates();
    ...
    function addCity()
    {
        ...
        {
            $this->load->model('city_model');
            //$data['roles'] = $this->city_model->getUserRoles();
            $data['tables'] = $this->city_model->showStates();
             
            $this->global['pageTitle'] = 'Waydadz : Add New State';


            $this->loadViews("addCity", $this->global, $data);
        }
    }
    ...
    function addNewCity()
    {
        ...
        else
        {
            ...
            $this->form_validation->set_rules('select_state','State Name','trim|required|max_length[128]');
            ...
            {
                $state = strtolower($this->security->xss_clean($this->input->post('select_state')));
                $city = strtolower($this->security->xss_clean($this->input->post('city')));
                //$school_list = ucwords(strtolower($this->security->xss_clean($this->input->post('slist'))));
                //$roleId = $this->input->post('role');
                //$mobile = $this->security->xss_clean($this->input->post('mobile'));
                
                //$state_city = array('state'=>$state, 'city' => $city);
                
                $this->load->model('city_model');
                $result = $this->city_model->addNewCity($state, $city);
    ...
    function addNewStates()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $this->load->model('city_model');
                $result = $this->city_model->addNewState($userInfo);
    ...
    function editOld($userId = NULL)
    {
        ...
        else
        {
            ...
            $data['roles'] = $this->city_model->getUserRoles();
            $data['userInfo'] = $this->city_model->getUserInfo($userId);
    ...
    function editUser()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                $result = $this->city_model->editUser($userInfo, $userId);
    ...
    function deleteUser()
    {
        ...
        else
        {
            ...
            $result = $this->city_model->deleteUser($userId, $userInfo);
